...
struct MonthView: View {
    ...
    // 뷰 높이의 58%를 달력 영역으로 사용, 헤더(~62pt) + 행 간격(5×4pt) 제외
    private var cellHeight: CGFloat {
        max(44, (viewHeight * 0.58 - 82) / 6)
    }
    ...
    var body: some View {
        ...
        .background(
            GeometryReader { geo in
                Color.clear
                    .onAppear { viewWidth = geo.size.width; viewHeight = geo.size.height }
                    .onChange(of: geo.size.width)  { viewWidth  = $0 }
                    .onChange(of: geo.size.height) { viewHeight = $0 }
            }
        )
    }
